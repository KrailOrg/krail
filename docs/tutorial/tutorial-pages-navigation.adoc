...
TIP: Extending ViewBase or one of its sub-classes is usually the easiest way to create your views, but however you do it, you must implement `KrailView`. `ViewBase` can also help with link:/userguide/userguide-serialisation.html[deserialization].
