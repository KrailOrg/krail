...
class DefaultServiceModelTest extends Specification {
    ...
    def "use Guice config to defined dependencies"() {
        ...
        then:
        assertThat(servicesModel.getClassGraph().findDependencies(sac, ServiceGraph.Selection.ALWAYS_REQUIRED)).containsOnly(sbc, scc)
        assertThat(servicesModel.getClassGraph().findDependencies(sac, ServiceGraph.Selection.ONLY_REQUIRED_AT_START)).containsOnly(sdc)
        assertThat(servicesModel.getClassGraph().findDependencies(sac, ServiceGraph.Selection.OPTIONAL)).containsOnly(sec)
        assertThat(servicesModel.getClassGraph().findDependencies(sbc, ServiceGraph.Selection.ONLY_REQUIRED_AT_START)).containsOnly(sfc)
        assertThat(servicesModel.getClassGraph().findDependencies(sgc, ServiceGraph.Selection.ALWAYS_REQUIRED)).containsOnly(sbc)
        assertThat(servicesModel.getClassGraph().findDependencies(shc, ServiceGraph.Selection.ALWAYS_REQUIRED)).containsOnly(sbc)
    }
