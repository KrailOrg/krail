...
class GuestMethodInterceptorTest extends Specification {

    GuestMethodInterceptor interceptor
    ...
    def setup() {
        ...
        interceptor = new GuestMethodInterceptor(subjectProviderProvider, annotationResolverProvider)
    }


    def "requires guest status, and is guest"() {
        given:
        RequiresGuest annotation = new InterceptorTestClass().getAnnotation('requiresGuest')
    ...
    def "requires guest status, and is not guest"() {
        given:
        RequiresGuest annotation = new InterceptorTestClass().getAnnotation('requiresGuest')
        subjectProvider.get() >> subject1
