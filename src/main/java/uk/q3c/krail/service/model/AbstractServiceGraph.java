...
/**
 * Default implementation of {@link ServiceGraph}
 * <p>
 * Created by David Sowerby on 03 Dec 2015
 */
@ThreadSafe
public abstract class AbstractServiceGraph<T> implements ServiceGraph<T> {
    private static final EnumMap<Selection, EnumSet<Dependency.Type>> selectionCriteria;
    private static Logger log = getLogger(AbstractServiceGraph.class);
    ...
    public synchronized void createDependency(T dependant, T dependency, Dependency.Type type) {
        ...
        if (hasDependency(dependant, dependency)) {
            throw new DuplicateDependencyException("There can only be one dependency between the same two service");
        }
