 ...
 * supports several variations by the use of {@link #sortOption} (the default is to sort the target nodes after creation
 ...
 * {@link TargetTreeWrapper#setNodeModifier(NodeModifier)} with an implementation to make the conversion. The default is
...
public class TreeCopy<S, T> {
	...
	private void loadNodeList(T parentNode, List<S> sourceNodeList, int level) {
		...
		// so the sort order must be determined before constructing target nodes. This could be used as default as it
		...
		case SORT_SOURCE_NODES:
			...
			break;
		// This is the default option, create the child nodes for a given parent, then sort them before adding them to
		// the target
		...
		// present, for example, a caption for MenuItem. Depending on the implementation, it may be easier to sort the
	...
	private void addNodesToTarget(T parentNode, Map<T, S> targetToSourceNodeMap) {
		if (target.getNodeModifier() == null || !target.getNodeModifier().attachOnCreate()) {
	...
	/**
	 * Now iterate over this level and drill down to next level If a node has no children call setLeaf (which the target
	...
	/**
	 * create the source nodes using target.addNode for this set of children. Target nodes are created using a map which
